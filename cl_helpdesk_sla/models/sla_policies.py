...
class HelpdeskSLA(models.Model):
    ...
    def cron_sla_reminder(self):
        """ Scheduled action to handle SLA Reminders. Checks tickets with SLA, calculates deadlines, and sends reminders if overdue.
        """
        _logger = logging.getLogger(__name__)
        Ticket = self.env['helpdesk.ticket']
        # Find all tickets with at least one SLA
        tickets = Ticket.search([('sla_ids', '!=', False), ('user_id', '!=', False), ('create_date', '!=', False)])
        ...
        for ticket in tickets:
            ...
            if fields.Datetime.from_string(now) >= deadline:
                ...
                if template and ticket.user_id:
                    ...
                    _logger.info(f"SLA Reminder sent for ticket {ticket.id} to user {ticket.user_id.id}")
                else:
                    _logger.warning(f"No email template or assigned user for ticket {ticket.id}")
