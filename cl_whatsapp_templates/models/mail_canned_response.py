...
class MailCannedResponse(models.Model):
    ...
    def render_substitution(self, channel_id=None):
        ...
        _logger.info(
            f"[Canned Response] render_substitution called for canned_response {self.id}, channel_id: {channel_id}"
        )
        ...
        _logger.info(
            f"[Canned Response] [USER] = {replacements['[USER]']} (user ID: {self.env.user.id})"
        )
        ...
        if channel_id:
            ...
            if channel.exists():
                ...
                if partner:
                    ...
                    _logger.info(
                        f"[Canned Response] [PHONE] = {replacements['[PHONE]']}"
                    )
                    _logger.info(
                        f"[Canned Response] [EMAIL] = {replacements['[EMAIL]']}"
                    )
                else:
                    # Fallback: Use channel display name if partner not found
                    replacements["[NAME]"] = channel.name or ""
                    _logger.warning(
                        f"[Canned Response] No partner found, using channel name: {channel.name}"
                    )
            else:
                _logger.warning(f"[Canned Response] Channel {channel_id} not found")
        else:
            _logger.warning(
                f"[Canned Response] No channel_id provided, skipping [NAME], [PHONE], [EMAIL]"
            )
        ...
        for var_num in variable_pattern:
            ...
            if var_index in replacements:
                substitution = substitution.replace(
                    f"{{{{{var_num}}}}}", replacements[var_index]
                )
            else:
                _logger.warning(
                    f"Variable {{{{var_num}}}} has no mapping, keeping as is"
                )
        ...
        if channel_id:
            ...
            if channel.exists():
                ...
                if partner:
                    ...
                    _logger.info(
                        f"[Canned Response Variables] Found partner: {partner.name}"
                    )
                else:
                    _logger.info(
                        f"[Canned Response Variables] No partner found for channel {channel.id}"
                    )
    ...
    def _find_partner_from_channel(self, channel):
        ...
        """
        _logger.info(
            f"[Canned Response] Finding partner for channel {channel.id} (type: {channel.channel_type})"
        )
        ...
        if channel.channel_type == "whatsapp":
            ...
            if whatsapp_number:
                ...
                if partner:
                    _logger.info(
                        f"[Canned Response] Found partner via phone: {partner.name} (ID: {partner.id})"
                    )
                    return partner
                else:
                    _logger.warning(
                        f"[Canned Response] No partner found for WhatsApp number: {whatsapp_number}"
                    )
            else:
                _logger.warning(
                    f"[Canned Response] WhatsApp channel has no whatsapp_number field"
                )
        ...
        if channel.channel_type == "chat":
            ...
            if other_members:
                ...
                _logger.info(
                    f"[Canned Response] Found partner from chat: {partner.name} (ID: {partner.id})"
                )
                return partner
        ...
        if channel.channel_partner_ids:
            ...
            if partners:
                partner = partners[0]
                _logger.info(
                    f"[Canned Response] Found partner from group: {partner.name} (ID: {partner.id})"
                )
                return partner
